...
void LRUReplacer::pin(frame_id_t frame_id) {
  ...
  if (it != LRUhash_.end()) {
    // remove the frame_id from the list
    LRUlist_.erase(it->second);
    // remove the frame_id from the hash map
    LRUhash_.erase(frame_id);
  }
}
...
void LRUReplacer::unpin(frame_id_t frame_id) {
  ...
  //  选择一个frame取消固定
    ...
    // check if the frame_id is already in the hash map
    if (LRUhash_.find(frame_id) != LRUhash_.end()) {
        return; // frame_id is already unpinned
    }
